...
import pickle
with open('dictionary_5.pickle', 'wb') as handle:
    pickle.dump(dictionary, handle, protocol=pickle.HIGHEST_PROTOCOL)
with open('ind2words_5.pickle', 'wb') as handle:
...
def generate_sample(index_words, context_window_size):
    file = open('center_context_pair_5.txt', 'w')
...
generate_sample(words2ind, 5)

file1=open('center_context_pair_5.txt','r')
i=0
context_dict=dict()
for i in range(vocab_size):
    context_dict[i]=set()
with open('center_context_pair_5.txt') as f:
    content = f.readlines()
    for line in content:
...
for epoch in tqdm(range(MAX_EPOCH)):
    with open('center_context_pair_5.txt') as train_read:
...
np.save('word_embeddings_d300_w5.npy',word_embeddings)
np.save('context_embeddings_d300_w5.npy',context_embeddings)
